...
class SchedulerReactive(config: WhiskConfig,
  ...
  val singleton = actorSystem.actorOf(SchedulerSingleton.props(config, poolConfig))
  ...
  def processActivationMessage(activationMessage: ActivationMessage): Unit = {
    ...
    if (!namespaceBlacklist.isBlacklisted(activationMessage.user)) {
        ...
        .flatMap { action =>
          action.toExecutableWhiskAction match {
            case Some(executable) =>
              ...
              Future.successful(())
            case None =>
              logging.error(this, s"non-executable action reached the scheduler ${action.fullyQualifiedName(false)}")
              Future.failed(new IllegalStateException("non-executable action reached the scheduler"))
          }
        }
        .recoverWith {
          case t =>
            ...
            // errors and should cause the scheduler to be considered unhealthy.
      ...
      ack(
        ...
        activationMessage.rootControllerIndex,
        activationMessage.user.namespace.uuid)
      logging.warn(this, s"namespace ${activationMessage.user.namespace.name} was blocked in scheduler.")
      Future.successful(())
    }
  }.recoverWith {
    case t =>
