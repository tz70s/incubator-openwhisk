...
/** Mentioned that, there's an existed Scheduler object in whisk.common, temporarily use it in fully qualified name. */
object Scheduler {
  ...
  def initKamon(): Unit = {
    ...
    val newKamonConfig = Kamon.config
      .withValue(
        ...
        ConfigValueFactory.fromAnyRef(s"scheduler"))
  ...
  def main(args: Array[String]): Unit = {
    ...
    implicit val actorSystem: ActorSystem =
      ActorSystem(name = "whisk-actor-system", defaultExecutionContext = Some(ec))
    ...
    initKamon()
    actorSystem.actorOf(SchedulerSingleton.props(config), name = "scheduler")
  }
}
