...
/**
 * The DockerClient instance is still one-to-one mapping here. No large logic got changed.
 * We'll have an upper level abstraction, to make all nodes transparent to align with Kube.
 *
 * The difference is, we should make Containers reachable behind NAT restriction.
 */
